                        ...
                        <div class="row mt-4">
                            <div class="col">
                                <div class="table-responsive">
                                    <table id="links-table" class="table" data-ajax_url="{{ route("admin.links.monthly") }}">
                                        <thead> 
                                            <tr>
...
@push('after-scripts')
    ...
    <script>
        ...
        let daysInMonth = Array.from({ length: <?php echo now()->daysInMonth; ?> }, (_, i) => i + 1);

        let links = daysInMonth.map(day => {
            let stat = linkStats.find(stat => stat.day === day);
            return stat ? stat.count : 0;
        });

        let views = daysInMonth.map(day => {
            let stat = viewStats.find(stat => stat.day === day);
            return stat ? stat.count : 0;
        });
